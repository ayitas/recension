...
1. The SDK **records** how a workflow behaves for each test case (checks, assumptions, metrics).
2. The server **collates** submissions across versions (batches).
3. Your team **promotes** a trusted batch as the baseline — the living recension.
4. Later versions are compared against that baseline; unintended side-effects appear as clearly as corrupted readings in a manuscript.
