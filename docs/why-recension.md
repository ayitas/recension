...
4. Later versions are compared against that baseline (or, in the dashboard, against another batch via compare-to); unintended side-effects — including slower timings — appear as clearly as corrupted readings in a manuscript.

This is not unit testing with hard-coded expected values. It is behavioral comparison against an authorized prior version.

See the root [`README.md`](../README.md) for stack, roles, invites, and OpenAPI; the dashboard element view shows metric duration bars vs baseline (or `?vs=`).
